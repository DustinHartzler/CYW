...
define('FOOBAR_FILE_CSS', 'foobar.2.6.css');
	...
	class FoobarNotifications extends WP_PluginBase_v1_5 {
		...
		function admin_settings_init() {
			...
			if (isset($upload_dir['baseurl'])) {
				...
				$upload_dir_baseurl = '<strong>NO PERMISSIONS</strong>';
			}
			...
			$this->admin_settings_add(array(
				...
				'choices' => array(
					...
					'generated' => 'Generate to Disk<br /><small>javascript and CSS code is saved to static files on the server, resulting in the best possible performance<br />Write access is required in order to create and update the static files.<br />Files are saved to <strong>' . $upload_dir_baseurl . '</strong><br /><strong>Please also note</strong> : shortcodes used within messages and custom HTML sections will not work with this option.</small>'
				)
			));
...
//          ) );
			...
			$this->admin_settings_add(array(
				...
				'title'   => __('Twitter Consumer Key'),
				'desc'    => __('The consumer key of your Twitter application.'),
				'type'    => 'text',
				'class'	  => 'long_textbox',
				'section' => 'Twitter OAuth V1.1',
				'tab'     => 'Twitter'
			));

			$this->admin_settings_add(array(
				'id'      => 'twitter_consumer_secret',
				'title'   => __('Twitter Consumer Secret'),
				'desc'    => __('The consumer secret of your Twitter application.'),
				'type'    => 'text',
				'class'	  => 'long_textbox',
				'section' => 'Twitter OAuth V1.1',
				'tab'     => 'Twitter'
			...
			$this->admin_settings_add(array(
				...
				'title'   => __('Twitter Access Key'),
				'desc'    => __('The access key of your Twitter application.'),
				'type'    => 'text',
				'class'	  => 'long_textbox',
				'section' => 'Twitter OAuth V1.1',
				'tab'     => 'Twitter'
			...
			$this->admin_settings_add(array(
				...
				'title'   => __('Twitter Access Secret'),
				'desc'    => __('The access secret of your Twitter application.'),
				'type'    => 'text',
				'class'	  => 'long_textbox',
				'section' => 'Twitter OAuth V1.1',
				'tab'     => 'Twitter'
		...
		function admin_cpt_foobar_css_enqueue() {
			...
			$cpt_metaboxes = $this->get_option('cpt_metabox_display', false);
			if ($cpt_metaboxes === false) {
				$cpt_defaults          = array();
			...
			foreach ($cpt_metaboxes as $posttype => $display_metabox) {
				if ($post_type == $posttype && $display_metabox == 'true') {
		...
		function admin_cpt_foobar_js_enqueue() {
			...
			if ($this->get_current_post_type() !== 'foobar') return;
...
//          }
		...
		function remove_admin_bar_foobar_links() {
			...
			if ($post_type !== 'foobar' &&
				(array_key_exists('page', $_GET) && $_GET['page'] !== 'foobar')
			) {
				return;
			}
...
//        }
		...
		function custom_admin_settings_render($args = array()) {
			...
			if ($type == 'social_profiles') {
				...
				<small>
					This is the default <?php echo $foobar_name; ?> that is shown on every page. You can also override
					which <?php echo $foobar_name; ?> is shown on individual posts and pages.<br/>
					Also, by setting a default, that <?php echo $foobar_name; ?> will be shown on every page, unless
					overridden per post or page.
				</small>
			<?php
			}
		}
		...
		function render_post_page_meta_box_content($post) {
			...
			?>
			<input type="hidden" name="foobar_post_nonce" id="foobar_post_nonce"
			...
			<table id="table_foobar_per_post_page" class="form-table">
				...
				<tr>
					<td colspan="2">
						<input class="foobar-radio rad_foobar_type" name="foobar_type" id="rad_foobar_type_default"
							<?php if ($foobar_type == "default") {
								echo 'checked="checked"';
							} ?> title="Let the default <?php echo $foobar_name; ?> be shown" type="radio"
							   value="default" tabindex="1"/><label for="rad_foobar_type_default">Show
							default <?php echo $foobar_name; ?></label>
						<input class="foobar-radio rad_foobar_type" name="foobar_type" id="rad_foobar_type_select"
							<?php if ($foobar_type == "select") {
								echo 'checked="checked"';
							} ?> title="Select which <?php echo $foobar_name; ?> you want shown" type="radio"
							   value="select" tabindex="2"/><label
							for="rad_foobar_type_select">Select <?php echo $foobar_name; ?></label>
						<input class="foobar-radio rad_foobar_type" name="foobar_type" id="rad_foobar_type_none"
							<?php if ($foobar_type == "none") {
								echo 'checked="checked"';
							} ?> title="Don't show any <?php echo $foobar_name; ?>" type="radio"
							   value="none" tabindex="2"/><label for="rad_foobar_type_none">Don't show
							a <?php echo $foobar_name; ?></label>
					</td>
					<td>
				</tr>
				</thead>
				<tbody>
				<tr class="foobar_type_row foobar_type_row_default<?php if ($foobar_type != "default") {
					echo " hidden";
				} ?>">
					<td colspan="2">
						The default <?php echo $foobar_name; ?> will be shown on this <?php echo $post->post_type; ?>
						.<br/>
						<?php if ($default_foobar && $default_foobar->ID > 0) { ?>
							That means <strong><?php echo $default_foobar->post_title; ?></strong> will be shown. <a
								href="edit.php?post_type=foobar">Change the default now</a>.
						<?php } else { ?>
							<strong>NO Default <?php echo $foobar_name; ?> has been set! This means
								no  <?php echo $foobar_name; ?> will be shown.</strong> <a
								href="options-general.php?page=foobar">Set the default now</a>.
						<?php } ?>
					</td>
				</tr>
				<tr class="foobar_type_row foobar_type_row_select<?php if ($foobar_type != "select") {
					echo " hidden";
				} ?>">
					<td colspan="2">
				...
				<tr class="foobar_type_row foobar_type_row_none<?php if ($foobar_type != "none") {
					echo " hidden";
				} ?>">
					<td colspan="2">
						A <?php echo $foobar_name; ?> will not be shown on this <?php echo $post->post_type; ?>. We
						promise!
					</td>
				</tr>
				</tbody>
		...
		function render_social_profile_form($socials, $row_class = '', $colspan = '2') {
			...
			<tr class="row_social_profiles <?php echo $row_class; ?>">
				<td colspan="<?php echo $colspan; ?>">
					...
					<p class="no_social_profiles<?php if ($socials !== false) {
						echo " hidden";
					} ?>">
						No social profiles have been added to this <?php echo $foobar_name; ?> yet. Please click the
						button below to add a new social profile.
					</p>
					<table class="widefat social_list<?php if ($socials === false) {
						echo " hidden";
					} ?>">
						<thead>
						<tr>
									...
									<td class="post-title page-title column-title">
										<div class="row_content">
											...
											<div class="social-row-actions">
												<span class="inline hide-if-no-js"><a href="#"
																					  class="editinline foobar_social_edit"
																					  title="Edit this item">Edit</a> | </span>
												<span class="trash"><a class="submitdelete foobar_social_delete"
																	   title="Delete this item"
																	   href="#">Delete</a></span>
											</div>
										</div>
										<div class="hidden">
											<input name="foobar[socials][<?php echo $index; ?>][name]"
												   class="txt_social_name" type="text"
												   value="<?php echo $social['name']; ?>"/>

											<p>
												<a href="#inline-edit-update" title="Update"
												   class="button-primary save alignleft foobar_social_update">Update</a>
												<a style="margin-left:10px" href="#inline-edit-cancel" title="Cancel"
												   class="button-secondary cancel alignleft foobar_social_cancel">Cancel</a>
											</p>
										</div>
									</td>
									<td>
										<div class="row_content"><img class="element_social_icon"
																	  src="<?php echo $social_img_icon ?>"/></div>
										<div class="hidden">
											<input name="foobar[socials][<?php echo $index; ?>][icon]"
												   class="txt_social_icon input_mid" type="text"
												   value="<?php echo $social['icon']; ?>"/>
											<a href="#" class="social_icon_select"><img title="select an existing icon"
																						border="0"
																						src="<?php echo $this->_plugin_url . 'images/icon_select.png'; ?>"/></a>
										</div>
									</td>
									<td>
										<div class="row_content"><span
												class="element_social_url"><?php echo $social['url']; ?></span></div>
										<div class="hidden">
											<input name="foobar[socials][<?php echo $index; ?>][url]"
												   class="txt_social_url input_mid" type="text"
												   value="<?php echo $social['url']; ?>"/>
										</div>
									</td>
								</tr>
							<?php
								...
								<div class="row_content">
									...
									<div class="social-row-actions">
										<span class="inline hide-if-no-js"><a href="#"
																			  ...
																			  title="Edit this item">Edit</a> | </span>
										<span class="trash"><a class="submitdelete foobar_social_delete"
															   title="Delete this item" href="#">Delete</a></span>
									</div>
								</div>
								<div class="hidden">
					...
					<p>
						<a href="#add" title="Add" class="button-secondary save alignleft foobar_social_add">Add Social
							Profile</a>
					</p>
				</td>
			</tr>
		...
		function render_foobar_options_meta_box_content($post) {
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<input type="text" name="foobar[backgroundColor]" class="foobar-colors" size="10"
					...
					<td width="250px">
						<input type="checkbox" name="foobar[forceTransparent]" class="foobar-checkbox-toggle"
							   data-textvisible="true" data-textchecked="Yes" data-leftwidth="40px"
							   data-textunchecked="No"
							   data-rightwidth="40px" <?php if ($forceTransparent == 'on') {
							echo " checked=\"checked\" ";
						} ?> />
						<br/>
						<small>Only use this if you are wanting to customize your <?php echo $foobar_name; ?> with a
				...
				<tr>
					...
					<td colspan="3">
						<input title="border color" type="text" name="foobar[borderColor]" class="foobar-colors"
							   size="10" value="<?php echo $borderColor; ?>"/>
						<input title="border thickness" size="5" name="foobar[borderSize]" class="foobar-num-slider"
							   data-slidewidth="80px" data-start="0" data-end="6" data-valueformat="{0}px" type="text"
							   value="<?php echo $borderSize; ?>"/>
						<br/>
				...
				<tr>
					...
					<td colspan="3">
						<input class="foobar-checkbox-toggle" data-textvisible="true" data-textchecked="Hide"
							   data-leftwidth="40px" data-textunchecked="Show"
							   data-rightwidth="40px" type="checkbox" <?php if ($hideShadow == 'on') {
							echo " checked=\"checked\" ";
						} ?> id="chkHideShadow" name="foobar[hideShadow]"/><br/>
				...
				<tr>
					...
					<td colspan="3">
						<input name="foobar[height]" size="8" data-slidewidth="250px" class="foobar-num-slider"
							   data-start="10" data-end="1000" data-step="1" data-valueformat="{0}px" type="text"
							   value="<?php echo $height; ?>"/>
						<br/>
						<small>The height of the <?php echo $foobar_name; ?> in pixels</small>
					</td>
				</tr>
				<tr>
					<td class="first-column">Collapsed Height</td>
					<td colspan="3">
						<input name="foobar[collapsedButtonHeight]" size="8" class="foobar-num-slider" data-start="10"
							   data-end="90" data-step="1" data-valueformat="{0}px" type="text"
							   value="<?php echo $collapsedButtonHeight; ?>"/>
						<br/>
						<small>The height of the <?php echo $foobar_name; ?> in pixels, when collapsed</small>
					</td>
				</tr>
				<tr>
					<td class="first-column">Positioning</td>
					<td colspan="3">
						<input class="foobar-radio" name="foobar[positioning]"
							   id="rad_position_fixed" <?php if ($positioning == "fixed") {
							echo 'checked="checked"';
						} ?> type="radio" value="fixed" tabindex="1"/><label class="padding-right"
																			 for="rad_position_fixed">Top</label>
						<input class="foobar-radio" name="foobar[positioning]" id="rad_position_inline"
							   <?php if ($positioning == "inline") {
								   echo 'checked="checked"';
							   } ?>type="radio" value="inline" tabindex="2"/><label class="padding-right"
																					for="rad_position_inline">Inline</label>
						<input class="foobar-radio" name="foobar[positioning]" id="rad_position_bottom"
							   <?php if ($positioning == "bottom") {
								   echo 'checked="checked"';
							   } ?>type="radio" value="bottom" tabindex="3"/><label
							for="rad_position_bottom">Bottom</label>
						<br/>
						<small>Top means the <?php echo $foobar_name; ?> is always fixed at the top of the page.<br/>
							Inline means the <?php echo $foobar_name; ?> will scroll out of view when the page is
							scrolled.<br/>
				...
				<tr>
					...
					<td colspan="3">
						<input class="foobar-radio" name="foobar[display]"
							   id="rad_display_expanded" <?php if ($display == "expanded") {
							echo 'checked="checked"';
						} ?> title="The default. The bar will be expanded when the page loads" type="radio"
							   value="expanded" tabindex="1"/><label class="padding-right" for="rad_display_expanded">Expanded</label>
						<input class="foobar-radio" name="foobar[display]"
							   id="rad_display_collapsed" <?php if ($display == "collapsed") {
							echo 'checked="checked"';
						} ?> title="The bar will be collapsed when the page loads" type="radio" value="collapsed"
							   tabindex="2"/><label class="padding-right" for="rad_display_collapsed">Collapsed</label>
						<input class="foobar-radio" name="foobar[display]"
							   id="rad_display_delayed" <?php if ($display == "delayed") {
							echo 'checked="checked"';
						} ?> title="The bar will be collapsed and will expand after a delay" type="radio"
							   value="delayed" tabindex="3"/><label class="padding-right" for="rad_display_delayed">Delayed</label>
						<input class="foobar-radio" name="foobar[display]"
							   id="rad_display_onscroll" <?php if ($display == "onscroll") {
							echo 'checked="checked"';
						} ?>
							   title="The bar will be collapsed and will only expand after the page has scrolled, after a delay"
							   type="radio" value="onscroll" tabindex="4"/><label for="rad_display_onscroll">On
							Scroll</label>
						<br/>
						<small>The initial state of the bar when the page loads. If set to <strong>Delayed</strong> or
							<strong>On Scroll</strong> then set the delay under Advanced Options
						</small>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<div class="hidden">
							<input class="foobar-radio" name="foobar[buttonTheme]"
								   id="rad_buttonTheme_default" <?php if ($buttonTheme == "triangle-arrow") {
								echo 'checked="checked"';
							} ?> type="radio" value="triangle-arrow" tabindex="1"/>
							<input class="foobar-radio" name="foobar[buttonTheme]"
								   id="rad_buttonTheme_long" <?php if ($buttonTheme == "long-arrow") {
								echo 'checked="checked"';
							} ?> type="radio" value="long-arrow" tabindex="2"/>
							<input class="foobar-radio" name="foobar[buttonTheme]"
								   id="rad_buttonTheme_white" <?php if ($buttonTheme == "small-white-arrow") {
								echo 'checked="checked"';
							} ?> type="radio" value="small-white-arrow" tabindex="3"/>
							<input class="foobar-radio" name="foobar[buttonTheme]"
								   id="rad_buttonTheme_close" <?php if ($buttonTheme == "x-close") {
								echo 'checked="checked"';
							} ?> type="radio" value="x-close" tabindex="4"/>
							<input class="foobar-radio" name="foobar[buttonTheme]"
								   id="rad_buttonTheme_xwhite" <?php if ($buttonTheme == "x-white") {
								echo 'checked="checked"';
							} ?> type="radio" value="x-white" tabindex="5"/>
						</div>
						<label class="button_theme_radio"
							   for="rad_buttonTheme_default"><a <?php if ($buttonTheme == "triangle-arrow") {
								echo 'class="selected"';
							} ?> href="#" title="Short"><img src="<?php echo $img_src_url_base; ?>/triangle-arrow.png"/></a></label>
						<label class="button_theme_radio"
							   for="rad_buttonTheme_long"><a <?php if ($buttonTheme == "long-arrow") {
								echo 'class="selected"';
							} ?> href="#" title="Long"><img src="<?php echo $img_src_url_base; ?>/long-arrow.png"/></a></label>
						<label class="button_theme_radio"
							   for="rad_buttonTheme_white"><a <?php if ($buttonTheme == "small-white-arrow") {
								echo 'class="selected"';
							} ?> href="#" title="Small White"><img
									src="<?php echo $img_src_url_base; ?>/small-white-arrow.png"/></a></label>
						<label class="button_theme_radio"
							   for="rad_buttonTheme_close"><a <?php if ($buttonTheme == "x-close") {
								echo 'class="selected"';
							} ?> href="#" title="Close X"><img src="<?php echo $img_src_url_base; ?>/x-close.png"/></a></label>
						<label class="button_theme_radio"
							   for="rad_buttonTheme_xwhite"><a <?php if ($buttonTheme == "x-white") {
								echo 'class="selected"';
							} ?> href="#" title="X White"><img src="<?php echo $img_src_url_base; ?>/x-white.png"/></a></label>
						<br/>
						<small>TIP : You can set the <?php echo $foobar_name; ?> to close when it is collapsed, rather
							than toggle (via Advanced Options). Setting it to close will allow your visitors to remove
							it from the page, and should probably be used in conjunction with the cookie settings.
						</small>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<div class="hidden">
							<input class="foobar-radio" name="foobar[navigationTheme]"
								   id="rad_navigationTheme_none" <?php if ($navigationTheme == "none") {
								echo 'checked="checked"';
							} ?> type="radio" value="none" tabindex="1"/>
							<input class="foobar-radio" name="foobar[navigationTheme]"
								   id="rad_navigationTheme_default" <?php if ($navigationTheme == "triangle-arrow") {
								echo 'checked="checked"';
							} ?> type="radio" value="triangle-arrow" tabindex="2"/>
							<input class="foobar-radio" name="foobar[navigationTheme]"
								   id="rad_navigationTheme_long" <?php if ($navigationTheme == "long-arrow") {
								echo 'checked="checked"';
							} ?> type="radio" value="long-arrow" tabindex="3"/>
							<input class="foobar-radio" name="foobar[navigationTheme]"
								   id="rad_navigationTheme_white" <?php if ($navigationTheme == "small-white-arrow") {
								echo 'checked="checked"';
							} ?> type="radio" value="small-white-arrow" tabindex="4"/>
							<input class="foobar-radio" name="foobar[navigationTheme]"
								   id="rad_navigationTheme_xwhite" <?php if ($navigationTheme == "x-white") {
								echo 'checked="checked"';
							} ?> type="radio" value="x-white" tabindex="5"/>
						</div>
						<label class="navigation_theme_radio"
							   for="rad_navigationTheme_none"><a <?php if ($navigationTheme == "none") {
								echo 'class="selected"';
							} ?> href="#" title="None"><img src="<?php echo $img_src_url_base; ?>/nav_theme_none.png"/></a></label>
						<label class="navigation_theme_radio"
							   for="rad_navigationTheme_default"><a <?php if ($navigationTheme == "triangle-arrow") {
								echo 'class="selected"';
							} ?> href="#" title="Short"><img
									src="<?php echo $img_src_url_base; ?>/triangle-arrow-nav.png"/></a></label>
						<label class="navigation_theme_radio"
							   for="rad_navigationTheme_long"><a <?php if ($navigationTheme == "long-arrow") {
								echo 'class="selected"';
							} ?> href="#" title="Long"><img src="<?php echo $img_src_url_base; ?>/long-arrow-nav.png"/></a></label>
						<label class="navigation_theme_radio"
							   for="rad_navigationTheme_white"><a <?php if ($navigationTheme == "small-white-arrow") {
								echo 'class="selected"';
							} ?> href="#" title="Small White"><img
									src="<?php echo $img_src_url_base; ?>/small-white-arrow-nav.png"/></a></label>
						<label class="navigation_theme_radio"
							   for="rad_navigationTheme_xwhite"><a <?php if ($navigationTheme == "x-white") {
								echo 'class="selected"';
							} ?> href="#" title="Small White Shadow"><img
									src="<?php echo $img_src_url_base; ?>/x-white-nav.png"/></a></label>
					</td>
				</tr>
			</table>
		<?php
		...
		function render_foobar_message_options_meta_box_content($post) {
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<input size="8" name="foobar[messagesDelay]" class="foobar-num-slider" data-slidewidth="80px"
							   data-start="0" data-end="20" data-step="1" data-snap="snap"
							   type="text" value="<?php echo $messagesDelay; ?>"/>
						<br/>
						<small>The delay (in seconds) between switching the messages (if more than 1 message is
							supplied)
						</small>
					</td>
					<td>
				...
				<tr>
					...
					<td>
						<input size="8" name="foobar[messagesFadeDelay]" class="foobar-num-slider"
							   data-slidewidth="80px" data-start="0" data-end="2000" data-step="100" data-snap="snap"
							   type="text" value="<?php echo $messagesFadeDelay; ?>"/>
						<br/>
						<small>The time (in milliseconds) it takes to transition to the next message (if more than 1
							message is supplied)
						</small>
					</td>
					<td>
				...
				<tr>
					...
					<td>
						<input size="8" name="foobar[messagesScrollSpeed]" class="foobar-num-slider"
							   data-slidewidth="80px" data-start="10" data-end="200" data-step="10" data-snap="snap"
							   type="text" value="<?php echo $messagesScrollSpeed; ?>"/>
						<br/>
						<small>The pixels per second to scroll extra length messages into view (if the message does not
							fit by default)
						</small>
					</td>
					<td>
				...
				<tr>
					...
					<td>
						<input size="8" name="foobar[messagesScrollDelay]" class="foobar-num-slider"
							   data-slidewidth="80px" data-start="0" data-end="10" data-step="1" data-snap="snap"
							   type="text" value="<?php echo $messagesScrollDelay; ?>"/>
						<br/>
						<small>The delay (in seconds) between initially displaying a long message and the beginning of
							scrolling it
						</small>
					</td>
					<td>
				...
				<tr>
					...
					<td>
						<input class="foobar-checkbox-toggle" data-textvisible="true" data-textchecked="On"
							   data-leftwidth="30px" data-textunchecked="Off" data-rightwidth="30px"
							   type="checkbox" <?php if ($enableRandom == 'on') {
							echo " checked=\"checked\" ";
						} ?>
				...
				<tr>
					...
					<td>
						<input class="foobar-checkbox-toggle" data-textvisible="true" data-textchecked="On"
							   data-leftwidth="30px" data-textunchecked="Off" data-rightwidth="30px"
							   type="checkbox" <?php if ($disableScrolling == 'on') {
							echo " checked=\"checked\" ";
						} ?>
    ...
    <td>
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<input class="foobar-checkbox-toggle" data-textvisible="true" data-textchecked="On"
							   ...
							   type="checkbox" <?php if ($enable_cookie == 'on') {
							echo " checked=\"checked\" ";
						} ?>
				...
				<tr>
					...
					<td>
						...
						<br/>
						<small>The name of the cookie as it is stored on the client machine. Change this value if you
							want to force the <?php echo $foobar_name; ?> to show.
						</small>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input size="3" name="foobar[cookieDuration]" type="text"
							   value="<?php echo $cookie_duration; ?>"/> days
						<br/>
		...
		function render_foobar_advanced_meta_box_content($post) {
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<input type="checkbox" data-textvisible="true" data-textchecked="Close"
							...
							<?php if ($buttonTypeClose == 'on') {
								echo " checked=\"checked\" ";
							} ?>
							   class="foobar-checkbox-toggle" name="foobar[buttonTypeClose]"/><br/>
						<small>If set to closed, the <?php echo $foobar_name; ?> will be closed when collapsed and
							cannot be expanded again.
						</small>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input size="10" name="foobar[speed]" class="foobar-num-slider" data-slidewidth="80px"
							   data-start="0" data-end="1000" data-step="100" data-snap="snap" data-format="{0}px"
							   type="text" value="<?php echo $speed; ?>"/>
						<br/>
						<small>This time (in milliseconds) of how quickly the <?php echo $foobar_name; ?> opens and
							collapses
						</small>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input size="10" name="foobar[displayDelay]" class="foobar-num-slider" data-slidewidth="80px"
							   data-start="0" data-end="30" data-step="1" data-snap="snap" data-format="{0} seconds"
							   type="text" value="<?php echo $displayDelay; ?>"/>
						<br/>
						<small>The time to wait (in seconds) before showing the <?php echo $foobar_name; ?>. Used in
							conjunction with the <strong>Delayed</strong> and <strong>On Scroll</strong> display options
						</small>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select name="foobar[easing]" id="drop_easing">
							<option <?php if ($easing == "swing") {
								echo 'selected="selected"';
							} ?> value="swing">Normal
							</option>
							<option <?php if ($easing == "easeOutSine") {
								echo 'selected="selected"';
							} ?> value="easeOutSine">Sine
							</option>
							<option <?php if ($easing == "easeOutElastic") {
								echo 'selected="selected"';
							} ?> value="easeOutElastic">Elastic
							</option>
							<option <?php if ($easing == "easeOutBack") {
								echo 'selected="selected"';
							} ?> value="easeOutBack">Back
							</option>
							<option <?php if ($easing == "easeOutBounce") {
								echo 'selected="selected"';
							} ?> value="easeOutBounce">Bounce
							</option>
						</select>
						<br/>
						<small>The type of animation used when expanding and collapsing
							the <?php echo $foobar_name; ?></small>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select name="foobar[positionClose]" id="drop_positionClose">
							<option <?php if ($positionClose == "left") {
								echo 'selected="selected"';
							} ?> value="left">Left
							</option>
							<option <?php if ($positionClose == "right") {
								echo 'selected="selected"';
							} ?> value="right">Right
							</option>
							<option <?php if ($positionClose == "hidden") {
								echo 'selected="selected"';
							} ?> value="hidden">Hidden
							</option>
						</select><br/>
				...
				<tr>
					...
					<td>
						<input type="checkbox" data-textvisible="true" data-textchecked="Ignore"
							...
							<?php if ($ignoreHtmlMarginTop == 'on') {
								echo " checked=\"checked\" ";
							} ?>
							   class="foobar-checkbox-toggle" name="foobar[ignoreHtmlMarginTop]"/><br/>
						<small>Ignore any HTML elements (including the Admin Bar) that set the top margin of the page.
							Setting this to ignore will force the <?php echo $foobar_name; ?> to be placed at the top of
							the page.
						</small>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="checkbox" data-textvisible="true" data-textchecked="On"
							   data-leftwidth="40px" data-textunchecked="Off" data-rightwidth="40px"
							<?php if ($disablePushDown == 'on') {
								echo " checked=\"checked\" ";
							} ?>
							   class="foobar-checkbox-toggle" name="foobar[disablePushDown]"/><br/>
						<small>By default, a <?php echo $foobar_name; ?> that is fixed at the top of the page, pushes
							the page down when it opens. To disable this, set this to option to <strong>On</strong>
						</small>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="checkbox" data-textvisible="true" data-textchecked="On"
							...
							<?php if ($rtl == 'on') {
							   ...
							   class="foobar-checkbox-toggle" name="foobar[rtl]"/><br/>
						<small>Change the alignment of the <?php echo $foobar_name; ?> to be right to left. This also
		...
		function render_foobar_twitter_meta_box_content($post) {
			...
			<table class="form-table">
				<tbody>
				<tr>
					<td colspan="2"><strong>PLEASE NOTE:</strong> in order to show your Tweets, you need to set your Twitter OAuth API settings on the
						<a href="options-general.php?page=<?php echo $this->_plugin_name; ?>#twitter"><?php echo $foobar_name; ?> settings page</a>.
					</td>
				</tr>
				<tr>
					<td class="first-column"><label for="chkEnableTwitter">Enable Twitter</label></td>
					<td>
						<input data-textvisible="true" data-textchecked="Enabled" data-textunchecked="Disabled"
							   data-leftwidth="60px" data-rightwidth="60px" class="foobar-checkbox-toggle"
							   type="checkbox" <?php if ($enable_twitter == "on") {
							echo " checked=\"checked\" ";
						} ?> id="chkEnableTwitter" name="foobar[twitterEnabled]"/><br/>
				...
				<tr>
					...
					<td>
						<input name="foobar[twitterMaxTweets]" class="foobar-num-slider" data-slidewidth="80px"
							   type="text" value="<?php echo $twitterMaxTweets; ?>"/>
						<br/>
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<input data-textvisible="true" data-textchecked="Enabled" data-textunchecked="Disabled"
							   data-leftwidth="60px" data-rightwidth="60px" class="foobar-checkbox-toggle"
							   type="checkbox" <?php if ($enable_rss == "on") {
							echo " checked=\"checked\" ";
						} ?> id="chkEnableRss" name="foobar[rssEnabled]"/><br/>
				...
				<tr>
					...
					<td>
						<input name="foobar[rssMaxResults]" class="foobar-num-slider" data-slidewidth="80px" type="text"
							   value="<?php echo $rssMaxResults; ?>"/>
						<br/>
						<small>The maximum number of RSS feed results to display in
							your <?php echo $foobar_name; ?></small>
					</td>
					<td>
				</tr>
				<tr>
		...
		function render_foobar_messages_meta_box_content($post) {
			...
			?>
			<table class="form-table widefat message_list" <?php if ($messages === false) {
							...
							<td>
								<div class="row_content">
									...
									<div class="message-row-actions">
										<span class="inline hide-if-no-js"><a href="#"
																			  class="editinline foobar_message_edit"
																			  title="Edit this message">Edit</a> | </span>
										<span class="trash"><a class="submitdelete foobar_message_delete"
															   title="Delete this message" href="#">Delete</a></span>
									</div>
								</div>
								<div class="hidden">
									<textarea name="foobar[messages][<?php echo $index; ?>]"
											  class="txt_message"><?php echo $message; ?></textarea>

									<p>
										<a href="#inline-edit-update" title="Update"
										   class="button-primary save alignleft foobar_message_update">Update</a>
										<a style="margin-left:10px" href="#inline-edit-cancel" title="Cancel"
										   class="button-secondary cancel alignleft foobar_message_cancel">Cancel</a>
									</p>
								</div>
							</td>
						</tr>
			...
			<p>
				<a href="#add" title="Add" class="button-secondary save alignleft foobar_message_add">Add New
					Message</a>
			</p>
				...
				<tr>
					...
					<td>
						<div class="row_content">
							...
							<div class="message-row-actions">
								<span class="inline hide-if-no-js"><a href="#" class="editinline foobar_message_edit"
																	  title="Edit this message">Edit</a> | </span>
								<span class="trash"><a class="submitdelete foobar_message_delete"
													   title="Delete this message" href="#">Delete</a></span>
							</div>
						...
						<div class="hidden">
							...
							<p>
								<a href="#inline-edit-update" title="Update"
								   class="button-secondary save alignleft foobar_message_update">Update</a>
								<a style="margin-left:10px" href="#inline-edit-cancel" title="Cancel"
								   class="button-secondary cancel alignleft foobar_message_cancel">Cancel</a>
							</p>
						</div>
		...
		function render_foobar_social_meta_box_content($post) {
			...
			<table class="form-table">
				<tr>
					...
					<td colspan="3">
						<select name="foobar[positionSocial]" id="drop_positionSocial">
							<option <?php if ($positionSocial == "left") {
								echo 'selected="selected"';
							} ?> value="left">Left
							</option>
							<option <?php if ($positionSocial == "right") {
								echo 'selected="selected"';
							} ?> value="right">Right
							</option>
							<option <?php if ($positionSocial == "hidden") {
								echo 'selected="selected"';
							} ?> value="hidden">Hidden
							</option>
						</select><br/>
						<small>Where do you want your social profiles positioned within the <?php echo $foobar_name; ?>
							?
						</small>
					</td>
				</tr>
				<tr>
					...
					<td valign="top">
						<input type="text" name="foobar[socialFontColor]" class="foobar-colors" size="10"
							   value="<?php echo $socialFontColor; ?>"/>
					</td>
				</tr>
				<tr>
					...
					<td>
						<select name="foobar[socialFontFamily]" id="drop_socialFontFamily">
							<option <?php if ($socialFontFamily == "Arial") {
								echo 'selected="selected"';
							} ?> value="Arial">Arial
							</option>
							<option <?php if ($socialFontFamily == "Verdana") {
								echo 'selected="selected"';
							} ?> value="Verdana">Verdana
							</option>
							<option <?php if ($socialFontFamily == "Tahoma") {
								echo 'selected="selected"';
							} ?> value="Tahoma">Tahoma
							</option>
							<option <?php if ($socialFontFamily == "Trebuchet MS") {
								echo 'selected="selected"';
							} ?> value="Trebuchet MS">Trebuchet MS
							</option>
							<option <?php if ($socialFontFamily == "Georgia") {
								echo 'selected="selected"';
							} ?> value="Georgia">Georgia
							</option>
							<option <?php if ($socialFontFamily == "Times New Roman") {
								echo 'selected="selected"';
							} ?> value="Times New Roman">Times New Roman
							</option>
							<option <?php if ($socialFontFamily == "Lucida Console") {
								echo 'selected="selected"';
							} ?> value="Lucida Console">Lucida Console
							</option>
							<option <?php if ($socialFontFamily == "Courier New") {
								echo 'selected="selected"';
							} ?> value="Courier New">Courier New
							</option>
							<option <?php if ($socialFontFamily == "Century Gothic") {
								echo 'selected="selected"';
							} ?> value="Century Gothic">Century Gothic
							</option>
							<option <?php if ($socialFontFamily == "MS Sans Serif") {
								echo 'selected="selected"';
							} ?> value="MS Sans Serif">MS Sans Serif
							</option>
						</select>
					</td>
					<td class="first-column">Social Text Size</td>
					<td>
						<input size="8" name="foobar[socialFontSize]" class="foobar-num-slider" data-valueformat="{0}pt"
							   data-slidewidth="80px" data-start="8" data-end="30" data-step="1" data-snap="snap"
							   data-format="{0}px" type="text" value="<?php echo $socialFontSize; ?>"/>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="checkbox" data-textvisible="true" data-textchecked="Yes" data-textunchecked="No"
							<?php if ($socialUseFontShadow == "on") {
								echo " checked=\"checked\" ";
							} ?>
							   data-leftwidth="40px" data-rightwidth="40px" class="foobar-checkbox-toggle"
							   name="foobar[socialUseFontShadow]"/>
					</td>
					<td class="first-column">Social Shadow Color</td>
					<td>
						<input type="text" name="foobar[socialFontShadow]" class="foobar-colors" size="10"
							   value="<?php echo $socialFontShadow; ?>"/>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3">
						<input data-textvisible="true" data-textchecked="Override" data-textunchecked="Use Defaults"
							   data-leftwidth="80px" data-rightwidth="60px" class="foobar-checkbox-toggle"
							   type="checkbox" <?php if ($overrideSocial) {
							echo " checked=\"checked\" ";
						} ?> id="chk_override_social" name="foobar[overrideSocial]"/><br/>
						<small>
							You can choose to override or inherit the default social profiles, so you only need to set
							them up once for all your <?php echo $plural; ?>.
							You can setup the default social profiles in the <a
								href="options-general.php?page=<?php echo $this->_plugin_name; ?>"><?php echo $foobar_name; ?>
								Settings</a>.
						</small>
					</td>
				</tr>
		...
		function render_foobar_publish_meta_box_content($post) {
			...
			<div class="submitbox" id="submitpost">
				<input type="hidden" name="foobar_nonce" id="foobar_nonce"
					   value="<?php echo wp_create_nonce(plugin_basename(__FILE__)); ?>"/>

				<div id="minor-publishing">
					<table class="form-table">
						<tr>
							<td class="first-column"><label
									for="chk_foobar_default"><?php echo __('Default'); ?></label></td>
							<td>
								<input data-textvisible="true" data-textchecked="Yes" data-textunchecked="No"
									   data-leftwidth="40px" data-rightwidth="40px" class="foobar-checkbox-toggle"
									   type="checkbox" <?php if ($default_foobar_id == $post->ID) {
									echo " checked=\"checked\" ";
								} ?>
									   id="chk_foobar_default" name="foobar_is_default"/><br/>
								<small>If you set this <?php echo $foobar_name; ?> to be the default, it will be
									displayed on all posts and pages.
								</small>
							</td>
						</tr>
						<tr>
							<td class="first-column"><label
									for="chk_foobar_preview"><?php echo __('Hide Preview'); ?></label></td>
							<td>
								<input data-textvisible="true" data-textchecked="Hide" data-textunchecked="Show"
									   data-leftwidth="40px" data-rightwidth="40px" class="foobar-checkbox-toggle"
									   type="checkbox" <?php if ($hidePreview == "on") {
									echo " checked=\"checked\" ";
								} ?>
									   id="chk_foobar_preview" name="foobar[hidePreview]"/><br/>
								<small>Hide the live functioning preview of the <?php echo $foobar_name; ?> on this
									page.
								</small>
							</td>
						</tr>
						<?php if ($this->get_option('foobar_enable_conditional_logic') == 'on') { ?>
							<tr>
								<td class="first-column"><label
										for="txt_foobar_advshow"><?php echo __('Conditional Logic'); ?></label></td>
								<td>
									<input type="text" value="<?php echo $conditional_logic; ?>" id="txt_foobar_advshow"
										   name="foobar[conditionalLogic]"/><br/>
									<small>Conditionally display the <?php echo $foobar_name; ?> based on <a
											href="http://codex.wordpress.org/Conditional_Tags" target="_blank">conditional
											tags</a></small>
								</td>
							</tr>
						<?php } ?>
						...
						if (!in_array($post->post_status, array('publish')) || 0 == $post->ID) {
							if ($can_publish) : ?>
								<input name="original_publish" type="hidden" id="original_publish"
									   value="<?php esc_attr_e('Create') ?>"/>
								<input name="publish" type="submit" class="button-primary" id="publish" tabindex="5"
									   accesskey="p" value="<?php echo __('Create'); ?>"/>
							...
							<input name="save" type="submit" class="button-primary" id="publish" tabindex="5"
		...
		function render_foobar_custom_css_box_content($post) {
			...
			<table id="table_styling" class="form-table">
				...
				<tr>
					...
					<td>
						<textarea class="textarea_xlarge" name="foobar[customCSS]"
								  type="text"><?php echo $customCSS; ?></textarea>
						<br/>
		...
		function render_foobar_custom_js_box_content($post) {
			...
			<table id="table_styling" class="form-table">
				...
				<tr>
					...
					<td>
						<textarea class="textarea_xlarge" name="foobar[customJS]"
								  type="text"><?php echo $customJS; ?></textarea>
						<br/>
						<small>Any custom javascript / jQuery that you want to be included with
							this <?php echo $foobar_name; ?></small>
					</td>
				</tr>
				</tbody>
			</table>
		...
		function render_foobar_styling_meta_box_content($post) {
			...
			<table id="table_styling" class="form-table">
				...
				<tr>
					<td colspan="2">
						<input class="foobar-radio rad_styling" name="foobar[styling]" id="rad_styling_custom"
							<?php if ($styling == "normal") {
								echo 'checked="checked"';
							} ?> title="" type="radio"
							   value="normal" tabindex="2"/><label for="rad_styling_custom">Normal Styling</label>
						<input class="foobar-radio rad_styling" name="foobar[styling]" id="rad_styling_customCSS"
							<?php if ($styling == "custom") {
								echo 'checked="checked"';
							} ?> title="" type="radio"
							   value="custom" tabindex="3"/><label for="rad_styling_customCSS">Custom CSS</label>
						<input class="foobar-radio rad_styling" name="foobar[styling]" id="rad_styling_none"
							<?php if ($styling == "none") {
								echo 'checked="checked"';
							} ?> title="" type="radio"
				...
				<tbody>
				<tr class="styling_row styling_row_normal<?php if ($styling != "normal") {
					echo " hidden";
				} ?>">
					<td colspan="2">
						<table class="widefat" style="width:690px !important">
							...
							<tr>
								...
								<td>
									<input type="text" name="foobar[fontColor]" class="foobar-colors" size="10"
										   value="<?php echo $fontColor; ?>"/>
								</td>
								<td>
									<input type="text" name="foobar[aFontColor]" class="foobar-colors" size="10"
										   value="<?php echo $aFontColor; ?>"/>
								</td>
								<td>
									<input type="text" name="foobar[aHoverFontColor]" class="foobar-colors" size="10"
										   value="<?php echo $aHoverFontColor; ?>"/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<select name="foobar[fontFamily]" id="drop_fontFamily">
										<option <?php if ($fontFamily == "Arial") {
											echo 'selected="selected"';
										} ?> value="Arial">Arial
										</option>
										<option <?php if ($fontFamily == "Verdana") {
											echo 'selected="selected"';
										} ?> value="Verdana">Verdana
										</option>
										<option <?php if ($fontFamily == "Tahoma") {
											echo 'selected="selected"';
										} ?> value="Tahoma">Tahoma
										</option>
										<option <?php if ($fontFamily == "Trebuchet MS") {
											echo 'selected="selected"';
										} ?> value="Trebuchet MS">Trebuchet MS
										</option>
										<option <?php if ($fontFamily == "Georgia") {
											echo 'selected="selected"';
										} ?> value="Georgia">Georgia
										</option>
										<option <?php if ($fontFamily == "Times New Roman") {
											echo 'selected="selected"';
										} ?> value="Times New Roman">Times New Roman
										</option>
										<option <?php if ($fontFamily == "Lucida Console") {
											echo 'selected="selected"';
										} ?> value="Lucida Console">Lucida Console
										</option>
										<option <?php if ($fontFamily == "Courier New") {
											echo 'selected="selected"';
										} ?> value="Courier New">Courier New
										</option>
										<option <?php if ($fontFamily == "Century Gothic") {
											echo 'selected="selected"';
										} ?> value="Century Gothic">Century Gothic
										</option>
										<option <?php if ($fontFamily == "MS Sans Serif") {
											echo 'selected="selected"';
										} ?> value="MS Sans Serif">MS Sans Serif
										</option>
									</select>
								...
								<td>
									<select name="foobar[aFontFamily]" id="drop_aFontFamily">
										<option <?php if ($aFontFamily == "Arial") {
											echo 'selected="selected"';
										} ?> value="Arial">Arial
										</option>
										<option <?php if ($aFontFamily == "Verdana") {
											echo 'selected="selected"';
										} ?> value="Verdana">Verdana
										</option>
										<option <?php if ($aFontFamily == "Tahoma") {
											echo 'selected="selected"';
										} ?> value="Tahoma">Tahoma
										</option>
										<option <?php if ($aFontFamily == "Trebuchet MS") {
											echo 'selected="selected"';
										} ?> value="Trebuchet MS">Trebuchet MS
										</option>
										<option <?php if ($aFontFamily == "Georgia") {
											echo 'selected="selected"';
										} ?> value="Georgia">Georgia
										</option>
										<option <?php if ($aFontFamily == "Times New Roman") {
											echo 'selected="selected"';
										} ?> value="Times New Roman">Times New Roman
										</option>
										<option <?php if ($aFontFamily == "Lucida Console") {
											echo 'selected="selected"';
										} ?> value="Lucida Console">Lucida Console
										</option>
										<option <?php if ($aFontFamily == "Courier New") {
											echo 'selected="selected"';
										} ?> value="Courier New">Courier New
										</option>
										<option <?php if ($aFontFamily == "Century Gothic") {
											echo 'selected="selected"';
										} ?> value="Century Gothic">Century Gothic
										</option>
										<option <?php if ($aFontFamily == "MS Sans Serif") {
											echo 'selected="selected"';
										} ?> value="MS Sans Serif">MS Sans Serif
										</option>
									</select>
							...
							<tr>
								...
								<td>
									<input size="8" name="foobar[fontSize]" class="foobar-num-slider"
										   data-valueformat="{0}pt" data-slidewidth="80px" data-start="8" data-end="30"
										   data-step="1" data-snap="snap" data-format="{0}px" type="text"
										   value="<?php echo $fontSize; ?>"/>
								</td>
								<td>
									<input size="8" name="foobar[aFontSize]" class="foobar-num-slider"
										   data-valueformat="{0}pt" data-slidewidth="80px" data-start="8" data-end="30"
										   data-step="1" data-snap="snap" data-format="{0}px" type="text"
										   value="<?php echo $aFontSize; ?>"/>
								</td>
								<td>&nbsp;</td>
							</tr>
							<tr>
								...
								<td>
									<input type="checkbox" data-textvisible="true" data-textchecked="Yes"
										   data-textunchecked="No"
										<?php if ($useFontShadow == "on") {
											echo " checked=\"checked\" ";
										} ?>
										   data-leftwidth="40px" data-rightwidth="40px" class="foobar-checkbox-toggle"
							...
							<tr>
								...
								<td>
									<input type="text" name="foobar[fontShadow]" class="foobar-colors" size="10"
										   value="<?php echo $fontShadow; ?>"/>
								</td>
								<td>
									<input type="text" name="foobar[aFontShadow]" class="foobar-colors" size="10"
										   value="<?php echo $aFontShadow; ?>"/>
								</td>
								<td>
									<input type="text" name="foobar[aHoverFontShadow]" class="foobar-colors" size="10"
										   value="<?php echo $aHoverFontShadow; ?>"/>
								</td>
							</tr>
							<tr>
								...
								<td>
									<select name="foobar[aFontDecoration]" id="drop_fontFamily">
										<option <?php if ($aFontDecoration == "none") {
											echo 'selected="selected"';
										} ?> value="none">none
										</option>
										<option <?php if ($aFontDecoration == "underline") {
											echo 'selected="selected"';
								...
								<td>
									<select name="foobar[aHoverFontDecoration]" id="drop_fontFamily">
										<option <?php if ($aHoverFontDecoration == "none") {
											echo 'selected="selected"';
										} ?> value="none">none
										</option>
										<option <?php if ($aHoverFontDecoration == "underline") {
											echo 'selected="selected"';
										} ?> value="underline">underline
										</option>
									</select>
				...
				<tr class="styling_row styling_row_custom<?php if ($styling != "custom") {
					echo " hidden";
				} ?>">
		...
		function render_foobar_widths_metabox_content($post) {
			...
			<table class="form-table">
				...
				<tr>
					<td colspan="2">
						<p>Widths for the <?php echo $foobar_name; ?> can be set using px (pixels), % (percentages) or
							*. Setting a width to * will auto fill the width to the remaining space.</p>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<br/>
						<small>Define a custom width for the left side of the <?php echo $foobar_name; ?>. (Default :
							*)
						</small>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<br/>
						<small>Define a custom width for the center message area of the <?php echo $foobar_name; ?>.
							(Default : 50%)
						</small>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						...
						<br/>
						<small>Define a custom width for the right side of the <?php echo $foobar_name; ?>. (Default :
							*)
						</small>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<br/>
						<small>Define a custom width for the close button of the <?php echo $foobar_name; ?>. (Default :
							80px)
						</small>
					</td>
				</tr>
			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						...
						<br/>
						<small>Any custom HTML that you want to append to the left side of
							the <?php echo $foobar_name; ?></small>
					</td>
					<td>
				</tr>
				<tr>
					...
					<td>
						...
						<br/>
						<small>Any custom HTML that you want to append to the right side of
							the <?php echo $foobar_name; ?></small>
					</td>
					<td>
				</tr>
		...
		function frontend_init() {
			...
			$references = $this->get_option('foobar_references', 'inline');

			if ($references == 'inline') {
				$where = 'wp_head';

				if ($this->get_option('foobar_scripts_in_footer') == 'on') {
...
//          }
		...
		function get_foobar_data($foobar_id) {
			if ($foobar_id > 0) {
				...
				if (is_null($foobar) ||
					($foobar && $foobar->post_status == 'trash')
				) {
					return false;
				}
		...
		function inline_dynamic_js() {
			...
			} else if ($this->get_option('foobar_references', 'inline') == 'inline') {
