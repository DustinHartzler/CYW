...
class WC_Memberships_User_Membership {
	...
	/**
	 * Ser the membership end datetime
	 *
	 * @since 1.0.0
	...
	public function add_note( $note, $notify = false ) {
		...
		if ( is_user_logged_in() && current_user_can( 'edit_post', $this->get_id() ) ) {
			...
			$comment_author       = __( 'WooCommerce', WC_Memberships::TEXT_DOMAIN );
			$comment_author_email = strtolower( __( 'WooCommerce', WC_Memberships::TEXT_DOMAIN ) ) . '@';
	...
	public function pause_membership( $note = null ) {
		...
		$this->update_status( 'paused', $note ? $note : __( 'Membership paused.', WC_Memberships::TEXT_DOMAIN ) );
	...
	public function cancel_membership( $note = null ) {
		$this->update_status( 'cancelled', $note ? $note : __( 'Membership cancelled.', WC_Memberships::TEXT_DOMAIN ) );
	}
	...
	public function activate_membership( $note = null ) {
		...
		if ( ! $note ) {
			$note = $this->is_paused() ?
							__( 'Membership resumed.', WC_Memberships::TEXT_DOMAIN ) :
							__( 'Membership activated.', WC_Memberships::TEXT_DOMAIN );
		}
