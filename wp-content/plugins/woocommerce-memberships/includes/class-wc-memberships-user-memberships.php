...
class WC_Memberships_User_Memberships {
	 ...
	 */
	public function get_user_memberships( $user_id = null ) {
	 ...
	 * @return null|WC_User_Membership
	 */
	...
	public function get_user_membership_statuses() {
		...
		$statuses = array(
			'wcm-active' => array(
				'label'       => _x( 'Active', 'Membership Status', WC_Memberships::TEXT_DOMAIN ),
				'label_count' => _n_noop( 'Active <span class="count">(%s)</span>', 'Active <span class="count">(%s)</span>', WC_Memberships::TEXT_DOMAIN ),
			),
			'wcm-complimentary' => array(
				'label'       => _x( 'Complimentary', 'Membership Status', WC_Memberships::TEXT_DOMAIN ),
				'label_count' => _n_noop( 'Complimentary <span class="count">(%s)</span>', 'Complimentary <span class="count">(%s)</span>', WC_Memberships::TEXT_DOMAIN ),
			),
			'wcm-pending' => array(
				'label'       => _x( 'Pending Cancellation', 'Membership Status', WC_Memberships::TEXT_DOMAIN ),
				'label_count' => _n_noop( 'Pending Cancellation <span class="count">(%s)</span>', 'Pending Cancellation <span class="count">(%s)</span>', WC_Memberships::TEXT_DOMAIN ),
			),
			'wcm-paused' => array(
				'label'       => _x( 'Paused', 'Membership Status', WC_Memberships::TEXT_DOMAIN ),
				'label_count' => _n_noop( 'Paused <span class="count">(%s)</span>', 'Paused <span class="count">(%s)</span>', WC_Memberships::TEXT_DOMAIN ),
			),
			'wcm-expired' => array(
				'label'       => _x( 'Expired', 'Membership Status', WC_Memberships::TEXT_DOMAIN ),
				'label_count' => _n_noop( 'Expired <span class="count">(%s)</span>', 'Expired <span class="count">(%s)</span>', WC_Memberships::TEXT_DOMAIN ),
			),
			'wcm-cancelled' => array(
				'label'       => _x( 'Cancelled', 'Membership Status', WC_Memberships::TEXT_DOMAIN ),
				'label_count' => _n_noop( 'Cancelled <span class="count">(%s)</span>', 'Cancelled <span class="count">(%s)</span>', WC_Memberships::TEXT_DOMAIN ),
			),
		);
	...
	public function expire_user_membership( $id ) {
		...
		$user_membership->update_status( 'expired', __( 'Membership expired.', WC_Memberships::TEXT_DOMAIN ) );
	}
	...
	public function handle_order_trashed( $post_id ) {
		...
		if ( $user_membership = $this->get_user_membership_by_order_id( $post_id ) ) {
			$user_membership->cancel_membership( __( 'Membership cancelled because the associated order was trashed.', WC_Memberships::TEXT_DOMAIN ) );
		}
	}
	...
	public function handle_order_refunded( $order_id ) {
		...
		if ( $user_membership = $this->get_user_membership_by_order_id( $order_id ) ) {
			$user_membership->cancel_membership( __( 'Membership cancelled because the associated order was refunded.', WC_Memberships::TEXT_DOMAIN ) );
		}
	}
