...
class WC_Memberships_Admin_Membership_Plans {
	...
	public function customize_columns( $columns ) {
		...
		$columns['slug']    = __( 'Slug', WC_Memberships::TEXT_DOMAIN );
		$columns['access']  = __( 'Access from', WC_Memberships::TEXT_DOMAIN );
		$columns['members'] = __( 'Members', WC_Memberships::TEXT_DOMAIN );
				...
				if ( ! empty( $product_ids ) ) {
					...
					foreach ( $product_ids as $product_id ) {
						...
						if ( is_object( $product ) ) {
							...
							printf( '<li><a href="%s">%s</a></li>', esc_url( $link ), $product->get_formatted_name() );
						}
					}
	...
	public function post_submitbox_misc_actions() {
		...
		<div class="misc-pub-section misc-pub-grant-access">
			<span class="grant-access">
				<?php esc_html_e( 'Existing purchases:', WC_Memberships::TEXT_DOMAIN ); ?>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'action', 'grant_access', get_edit_post_link( $post->ID ) ), 'wc-memberships-grant-access-plan_' . $post->ID ) ); ?>" class="button" id="grant-access"><?php esc_html_e( 'Grant Access', WC_Memberships::TEXT_DOMAIN ); ?></a>
			</span>
		</div>
	...
	public function duplicate_button() {
		...
		if ( isset( $_GET['post'] ) ) {
			...
			<div id="duplicate-action">
				<a class="submitduplicate duplication" href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( 'Make a copy', WC_Memberships::TEXT_DOMAIN ); ?></a>
			</div>
			<?php
		}
	}
	...
	public function duplicate_membership_plan() {
		...
		if ( ! empty( $post ) ) {
			...
			wc_memberships()->admin->message_handler->add_message( __( 'Membership plan copied.', WC_Memberships::TEXT_DOMAIN ) );
			...
			wp_die( __( 'Membership plan creation failed, could not find original product:', WC_Memberships::TEXT_DOMAIN ) . ' ' . $id );
		}
	}
	...
	public function duplicate_plan( $post, $parent = 0, $post_status = 'publish' ) {
		...
		if ( $parent > 0 ) {
			...
			$suffix      = ' ' . __( '(Copy)', WC_Memberships::TEXT_DOMAIN );
		}
