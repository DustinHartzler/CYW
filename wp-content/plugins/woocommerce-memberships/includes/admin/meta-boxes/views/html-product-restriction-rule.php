...
<tr>
	<td class="check-column">
		<p class="form-field">
			<label for="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_checkbox"><?php esc_html_e( 'Select', WC_Memberships::TEXT_DOMAIN ); ?>:</label>
			<?php if ( $rule->current_user_can_edit() && $rule->current_context_allows_editing() ) : ?>
			<input type="checkbox" id="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_checkbox"  />
			<?php endif; ?>
			<input type="hidden" name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][membership_plan_id]" value="<?php echo esc_attr( $rule->get_membership_plan_id() ); ?>" />
			<input type="hidden" name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][id]" class="js-rule-id" value="<?php echo esc_attr( $rule->get_id() ); ?>" />
			<input type="hidden" name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][remove]" class="js-rule-remove" value="" />
			<?php if ( $rule->get_membership_plan_id() != $post->ID && $rule->has_objects() ) : ?>
				<?php foreach ( $rule->get_object_ids() as $id ) : ?>
				<input type="hidden" name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][object_ids][]" value="<?php echo esc_attr( $id ); ?>" />
				<?php endforeach; ?>
			<?php endif; ?>
		</p>
	</td>
	...
	<td class="product-restriction-content-type content-type-column">
		<p class="form-field">
			<label for="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_content_type_key"><?php esc_html_e( 'Type', WC_Memberships::TEXT_DOMAIN ); ?>:</label>
			<select name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][content_type_key]" id="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_content_type_key" class="js-content-type" <?php if ( ! $rule->current_user_can_edit() ) : ?>disabled<?php endif; ?>>
	...
	<td class="product-restriction-objects objects-column">
		<p class="form-field">
			<label for="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_object_ids"><?php esc_html_e( 'Title', WC_Memberships::TEXT_DOMAIN ); ?>:</label>
			<?php if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_3() ) : ?>
			<input type="hidden" class="wc-memberships-object-search js-object-ids" style="width: 50%;" id="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_object_ids" name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][object_ids]" data-placeholder="<?php esc_attr_e( 'Search&hellip; or leave blank to apply to all', WC_Memberships::TEXT_DOMAIN ); ?>" data-action="<?php echo esc_attr( $rule->get_object_search_action_name() ); ?>" data-multiple="true" data-selected="<?php
				$json_ids    = array();
		...
		<?php else : ?>
			<select name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][object_ids][]" id="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_object_ids" class="ajax_chosen_select_objects js-object-ids" multiple="multiple" data-placeholder="<?php esc_attr_e( 'Search&hellip; or leave blank to apply to all', WC_Memberships::TEXT_DOMAIN ); ?>" <?php if ( ! $rule->current_user_can_edit() ) : ?>disabled<?php endif; ?>>
				<?php
					if ( $rule->has_objects() ) {
  ...
  <td class="product-restriction-membership-plan membership-plan-column">
    <p class="form-field">
      <label for="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_membership_plan_id"><?php esc_html_e( 'Plan', WC_Memberships::TEXT_DOMAIN ); ?>:</label>
      <select name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][membership_plan_id]" id="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_membership_plan_id" <?php if ( ! $rule->current_user_can_edit() || ! $rule->current_context_allows_editing() ) : ?>disabled<?php endif; ?>>
  ...
  <?php endif; ?>
	...
	<td class="product-restriction-access-type access-type-column">
		<p class="form-field">
			<label for="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_access_type"><?php esc_html_e( 'Only Members Can', WC_Memberships::TEXT_DOMAIN ); ?>:</label>
			<select name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][access_type]" id="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_access_type" <?php if ( ! $rule->current_user_can_edit() || ! $rule->current_context_allows_editing() ) : ?>disabled<?php endif; ?>>
	...
	<td class="product-restriction-access-schedule access-schedule-column">
		<p class="form-field">
			...
			<span class="rule-control-group rule-control-group-access-schedule-specific js-hide-if-access-schedule-immediate <?php if ( 'immediate' == $rule->get_access_schedule() ) : ?>hide<?php endif;?>">
				...
				<?php ob_start(); ?>
				<input type="number" id="_product_restriction_rules_<?php echo esc_attr( $index ); ?>_access_schedule_amount" name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][access_schedule_amount]" min="0" class="access_schedule-amount" value="<?php echo esc_attr( $rule->get_access_schedule_amount() ); ?>" <?php if ( ! $rule->current_user_can_edit() || ! $rule->current_context_allows_editing() ) : ?>disabled<?php endif; ?> />
				<?php $amount = ob_get_clean(); ?>

				<?php ob_start(); ?>
				<select name="_product_restriction_rules[<?php echo esc_attr( $index ); ?>][access_schedule_period]" class="access_schedule-period js-access-schedule-period-selector" <?php if ( ! $rule->current_user_can_edit() || ! $rule->current_context_allows_editing() ) : ?>disabled<?php endif; ?>>
...
<tr class="disabled-notice">
	...
	<?php if ( ! $rule->current_user_can_edit() ) : ?>
		<span class="description"><?php esc_html_e( "You are not allowed to edit this rule", WC_Memberships::TEXT_DOMAIN ); ?></span>
	<?php else : ?>
		<span class="description"><?php printf( esc_html__( "This rule cannot be edited here because it applies to multiple products. You can %sedit this rule on the membership plan screen%s.", WC_Memberships::TEXT_DOMAIN ), '<a href="' . esc_url( get_edit_post_link( $rule->get_membership_plan_id() ) ) . '">', '</a>' ); ?></span>
	<?php endif; ?>
	</td>
</tr>
<?php endif; ?>
