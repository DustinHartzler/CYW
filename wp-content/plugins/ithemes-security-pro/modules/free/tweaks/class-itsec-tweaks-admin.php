...
class ITSEC_Tweaks_Admin {
	...
	public function tweaks_wordpress_generator_tag() {
		...
		$content .= '<label for="itsec_tweaks_server_generator_tag">' . __( 'Remove WordPress Generator Meta Tag', 'it-l10n-ithemes-security-pro' ) . '</label>';
		$content .= '<p class="description">' . __( 'Removes the <code>&lt;meta name="generator" content="WordPress [version]" /&gt;</code></pre> meta tag from your sites header. This process hides version information from a potential attacker making it more difficult to determine vulnerabilities.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function tweaks_wordpress_random_version() {
		...
		$content .= '<label for="itsec_tweaks_server_random_version">' . __( 'Display Random Version', 'it-l10n-ithemes-security-pro' ) . '</label>';
		$content .= '<p class="description">' . __( 'Where a WordPress version must be displayed, it will display a random WordPress version and will remove the WordPress version completely where possible.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function tweaks_wordpress_safe_jquery() {
		...
		if ( $raw_version !== false ) {
			...
			$version = sprintf(
				...
				__( 'undetermined. Please', 'it-l10n-ithemes-security-pro' ),
				site_url(),
				__( 'check your homepage', 'it-l10n-ithemes-security-pro' ),
				__( 'to see if you even need this feature', 'it-l10n-ithemes-security-pro' )
			);
		}
print site_url();
		if ( $is_safe === true ) {
		...
		echo '<p class="description" style="color: ' . $color . '">' . __( 'Your current jQuery version is ', 'it-l10n-ithemes-security-pro' ) . $version . '.</p>';
		printf(
	...
	public function dashboard_status( $statuses ) {
		...
		if ( isset( $this->settings['generator_tag'] ) && $this->settings['generator_tag'] === true ) {
			...
			$status_array = 'safe-medium';
			$status       = array(
				'text' => __( 'Your WordPress installation is not telling every bot that you use WordPress.', 'it-l10n-ithemes-security-pro' ),
			...
			$status_array = 'medium';
			$status       = array(
				'text' => __( 'Your WordPress installation is telling every bot that you use WordPress with a special "generator" tag. Click here to fix this.', 'it-l10n-ithemes-security-pro' ),
				'link' => '#itsec_tweaks_server_generator_tag',
			);
		...
		if ( isset( $this->settings['random_version'] ) && $this->settings['random_version'] === true ) {
			...
			$status_array = 'safe-low';
			$status       = array(
				'text' => __( 'Version information is obscured to all non admin users.', 'it-l10n-ithemes-security-pro' ),
			...
			$status_array = 'low';
			$status       = array(
				'text' => __( 'Users may still be able to get version information from various plugins and themes. Click here to fix this.', 'it-l10n-ithemes-security-pro' ),
				'link' => '#itsec_tweaks_server_random_version',
			);
	...
	public function initialize_admin() {
		...
		add_settings_field(
			...
			__( 'Generator Meta Tag', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_field(
			...
			__( 'Display Random Version', 'it-l10n-ithemes-security-pro' ),
