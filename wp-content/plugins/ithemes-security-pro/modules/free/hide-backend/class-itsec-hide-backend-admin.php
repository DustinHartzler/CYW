...
class ITSEC_Hide_Backend_Admin {
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array(
				'text' => __( 'Your WordPress Dashboard is hidden.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_hide_backend_enabled',
			);
			...
			$status_array = 'medium';
			$status       = array(
				'text' => __( 'Your WordPress Dashboard is using the default addresses. This can make a brute force attack much easier.', 'it-l10n-ithemes-security-pro' ),
	...
	public function hide_backend_slug() {
		...
		if ( ( get_option( 'permalink_structure' ) == '' || get_option( 'permalink_structure' ) == false ) && ! is_multisite() ) {
			...
			$content .= '<label for="itsec_hide_backend_strong_passwords_slug">' . __( 'Login URL:', 'it-l10n-ithemes-security-pro' ) . trailingslashit( get_option( 'siteurl' ) ) . '<span style="color: #4AA02C">' . sanitize_title( $this->settings['slug'] ) . '</span></label>';
			$content .= '<p class="description">' . __( 'The login url slug cannot be "login," "admin," "dashboard," or "wp-login.php" as these are use by default in WordPress.', 'it-l10n-ithemes-security-pro' ) . '</p>';
			$content .= '<p class="description"><em>' . __( 'Note: The output is limited to alphanumeric characters, underscore (_) and dash (-). Special characters such as "." and "/" are not allowed and will be converted in the same manner as a post title. Please review your selection before logging out.', 'it-l10n-ithemes-security-pro' ) . '</em></p>';
	...
	public function hide_backend_theme_compat_slug() {
		...
		if ( ( get_option( 'permalink_structure' ) == '' || get_option( 'permalink_structure' ) == false ) && ! is_multisite() ) {
			...
			$content .= '<label for="itsec_hide_backend_strong_passwords_theme_compat_slug">' . __( '404 Slug:', 'it-l10n-ithemes-security-pro' ) . trailingslashit( get_option( 'siteurl' ) ) . '<span style="color: #4AA02C">' . $slug . '</span></label>';
			$content .= '<p class="description">' . __( 'The slug to redirect folks to when theme compatibility mode is enabled (just make sure it does not exist in your site).', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function hide_backend_post_logout_slug() {
		...
		if ( ( get_option( 'permalink_structure' ) == '' || get_option( 'permalink_structure' ) == false ) && ! is_multisite() ) {
			...
			$content .= '<label for="itsec_hide_backend_strong_passwords_post_logout_slug">' . __( 'Custom Action:', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function hide_backend_theme_compat() {
		...
		if ( ( get_option( 'permalink_structure' ) == '' || get_option( 'permalink_structure' ) == false ) && ! is_multisite() ) {
			...
			$content .= '<label for="itsec_hide_backend_theme_compat"> ' . __( 'Enable theme compatibility. If  you see errors in your theme when using hide backend, in particular when going to wp-admin while not logged in, turn this on to fix them.', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function hide_backend_register() {
		...
		if ( ( get_option( 'permalink_structure' ) == '' || get_option( 'permalink_structure' ) == false ) && ! is_multisite() ) {
			...
			$content .= '<label for="itsec_hide_backend_strong_passwords_register">' . __( 'Registration URL:', 'it-l10n-ithemes-security-pro' ) . trailingslashit( get_option( 'siteurl' ) ) . '<span style="color: #4AA02C">' . sanitize_title( $this->settings['register'] ) . '</span></label>';
	...
	public function initialize_admin() {
		...
		add_settings_section(
			...
			__( 'Hide Login and Admin', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_field(
			...
			__( 'Enable Theme Compatibility', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_field(
			...
			__( 'Theme Compatibility Slug', 'it-l10n-ithemes-security-pro' ),
	...
	public function register_tooltip( $tooltip_modules ) {
		...
		if ( get_site_transient( 'ITSEC_SHOW_HIDE_BACKEND_TOOLTIP' ) || ( isset( $this->settings['show-tooltip'] ) && $this->settings['show-tooltip'] === true ) ) {
			...
			$tooltip_modules['hide-backend'] = array(
				...
				'heading'   => __( 'Review Hide Backend Settings', 'it-l10n-ithemes-security-pro' ),
				'text'      => __( 'The hide backend system has been rewritten. You must re-activate the feature to continue using the feature.', 'it-l10n-ithemes-security-pro' ),
				'link_text' => __( 'Review Settings', 'it-l10n-ithemes-security-pro' ),
	...
	private function send_new_slug( $new_slug ) {
		...
		$body = sprintf(
			...
			__( 'This friendly email is just a reminder that you have changed the dashboard login address on', 'it-l10n-ithemes-security-pro' ),
			get_site_url(),
			get_site_url(),
			__( 'You must now use', 'it-l10n-ithemes-security-pro' ),
			$new_slug,
			$new_slug,
			__( 'to login to your WordPress website', 'it-l10n-ithemes-security-pro' )
		);

		//Setup the remainder of the email
		$recipients = $itsec_globals['settings']['notification_email'];
		$subject    = '[' . get_option( 'siteurl' ) . '] ' . __( 'WordPress Login Address Changed', 'it-l10n-ithemes-security-pro' );
