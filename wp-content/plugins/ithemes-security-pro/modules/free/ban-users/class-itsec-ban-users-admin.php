...
class ITSEC_Ban_Users_Admin {
	...
	public function add_admin_meta_boxes() {
		...
		$title = __( 'Banned Users', 'it-l10n-ithemes-security-pro' );
	...
	public function ban_users_agent_list() {
		...
		$content .= '<p>' . __( 'Use the guidelines below to enter user agents that will not be allowed access to your site.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		$content .= '<ul>';
		$content .= '<li>' . __( 'Enter only 1 user agent per line.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		$content .= '</ul>';
	...
	public function ban_users_default() {
		...
		$content .= '<label for="itsec_ban_users_default"> ' . __( 'Enable HackRepair.com\'s blacklist feature', 'it-l10n-ithemes-security-pro' ) . '</label>';
		$content .= '<p class="description">' . __( 'As a getting-started point you can include the excellent blacklist developed by Jim Walker of <a href="http://hackrepair.com/blog/how-to-block-bots-from-seeing-your-website-bad-bots-and-drive-by-hacks-explained" target="_blank">HackRepair.com</a>.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function ban_users_enabled() {
		...
		$content .= '<label for="itsec_ban_users_enabled"> ' . __( 'Enable ban users', 'it-l10n-ithemes-security-pro' ) . '</label>';
	...
	public function ban_users_host_list() {
		...
		echo '<p>' . __( 'Use the guidelines below to enter hosts that will not be allowed access to your site. Note you cannot ban yourself.', 'it-l10n-ithemes-security-pro' ) . '</p>';
		echo '<ul>';
		echo '<li>' . __( 'You may ban users by individual IP address or IP address range.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '<li>' . __( 'Individual IP addesses must be in IPV4 standard format (i.e. ###.###.###.### or ###.###.###.###/##). Wildcards (*) or a netmask is allowed to specify a range of ip addresses.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '<li>' . __( 'If using a wildcard (*) you must start with the right-most number in the ip field. For example ###.###.###.* and ###.###.*.* are permitted but ###.###.*.### is not.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '<li><a href="http://ip-lookup.net/domain-lookup.php" target="_blank">' . __( 'Lookup IP Address.', 'it-l10n-ithemes-security-pro' ) . '</a></li>';
		echo '<li>' . __( 'Enter only 1 IP address or 1 IP address range per line.', 'it-l10n-ithemes-security-pro' ) . '</li>';
		echo '</ul>';
	...
	public function dashboard_status( $statuses ) {
		...
		if ( $this->settings['enabled'] === true ) {
			...
			$status       = array(
				'text' => __( 'You are blocking known bad hosts and agents with the ban users tool.', 'it-l10n-ithemes-security-pro' ),
			...
			$status_array = 'low';
			$status       = array(
				'text' => __( 'You are not blocking any users that are known to be a problem. Consider turning on the Ban Users feature.', 'it-l10n-ithemes-security-pro' ),
				'link' => '#itsec_ban_users_enabled',
			);
	...
	public function initialize_admin() {
		...
		add_settings_section(
			'ban_users_default',
			__( 'Default Blacklist', 'it-l10n-ithemes-security-pro' ),
		...
		add_settings_section(
			'ban_users_enabled',
			__( 'Configure Ban Users', 'it-l10n-ithemes-security-pro' ),
			'__return_empty_string',
			'security_page_toplevel_page_itsec_settings'
		);
		...
		add_settings_field(
			...
			__( 'Ban Hosts', 'it-l10n-ithemes-security-pro' ),
	...
	public function metabox_advanced_settings() {
		...
		echo '<p>' . __( 'This feature allows you to completely ban hosts and user agents from your site without having to manage any configuration of your server. Any IP addresses or user agents found in the lists below will not be allowed any access to your site.', 'it-l10n-ithemes-security-pro' ) . '</p>';
	...
	public function sanitize_module_input( $input ) {
		...
		if ( sizeof( $bad_ips ) > 0 ) {
			...
			if ( $no_errors === true ) {
				$message = sprintf( '%s<br /><br />', __( 'Note that the ban users feature has been disabled until the following errors are corrected:', 'it-l10n-ithemes-security-pro' ) );
			}

			foreach ( $bad_ips as $bad_ip ) {
				$message .= sprintf( '%s %s<br />', $bad_ip, __( 'is not a valid address in the ban users box.', 'it-l10n-ithemes-security-pro' ) );
			}
		...
		if ( sizeof( $white_ips ) > 0 ) {
			...
			foreach ( $white_ips as $white_ip ) {
				$message .= sprintf( '%s %s<br />', $white_ip, __( 'is not a valid address as it has been white listed.', 'it-l10n-ithemes-security-pro' ) );
			}
