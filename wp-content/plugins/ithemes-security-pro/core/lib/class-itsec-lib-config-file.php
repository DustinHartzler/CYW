...
class ITSEC_Lib_Config_File {
	...
	protected static function update( $file, $type, $modification, $clear_existing_modifications = true ) {
		...
		if ( false === apply_filters( 'itsec_filter_can_write_to_files', false ) ) {
			...
			return new WP_Error( 'itsec-file-writes-are-disabled', sprintf( __( 'The "Write to Files" setting is disabled. Manual configuration for the <code>%s</code> file can be found on the Security > Dashboard page.', 'it-l10n-ithemes-security-pro' ), $display_file ) );
		}
	...
	protected static function strip_php_comments( $contents ) {
		if ( ! ITSEC_Lib_Utility::is_callable_function( 'token_get_all' ) ) {
			return new WP_Error( 'itsec-lib-config-file-strip-php-comments-token-get-all-is-disabled', __( 'Unable to strip comments from the source code as the token_get_all() function is disabled. This is a system configuration issue.', 'it-l10n-ithemes-security-pro' ) );
		}
		...
		if ( ! is_array( $tokens ) ) {
			return new WP_Error( 'itsec-lib-config-file-strip-php-comments-token-get-all-invalid-response', sprintf( __( 'Unable to strip comments from the source code as the token_get_all() function returned an unrecognized value (type: %s)', 'it-l10n-ithemes-security-pro' ), gettype( $tokens ) ) );
		}
