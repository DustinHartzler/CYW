...
final class ITSEC_Files {
	...
	function add_admin_meta_boxes() {
		...
		add_meta_box(
			'itsec_rewrite',
			__( 'Rewrite Rules', 'it-l10n-ithemes-security-pro' ),
			...
			if ( isset( $itsec_globals['settings']['write_files'] ) && true === $itsec_globals['settings']['write_files'] ) {
				...
				if ( is_array( $rewrites ) ) {
					if ( false === $rewrites['success'] ) {
						...
						$message = sprintf( __( 'Unable to update the <code>%1$s</code> file. You may need to manually remove the existing iThemes Security modifications and replace them with the rules found at <a href="%2$s">Security > Dashboard</a> under the "Rewrite Rules" section.', 'it-l10n-ithemes-security-pro' ), $file, admin_url( 'admin.php?page=itsec#itsec_rewrite' ) );
						add_settings_error( 'itsec', esc_attr( 'settings_updated' ), $message, 'error' );
					} else if ( true !== $rewrites['text'] ) {
						add_settings_error( 'itsec', esc_attr( 'settings_updated' ), __( 'Settings Updated', 'it-l10n-ithemes-security-pro' ) . '<br />' . $rewrites['text'], 'updated' );
					}
				} else {
			...
			if ( isset( $itsec_globals['settings']['write_files'] ) && true === $itsec_globals['settings']['write_files'] ) {
				...
				if ( is_array( $configs ) ) {
					if ( false === $configs['success'] ) {
						...
						$message = sprintf( __( 'Unable to update the <code>%1$s</code> file. You may need to manually remove the existing iThemes Security modifications and replace them with the rules found at <a href="%2$s">Security > Dashboard</a> under the "wp-config.php Rules" section.', 'it-l10n-ithemes-security-pro' ), ABSPATH . 'wp-config.php', admin_url( 'admin.php?page=itsec#itsec_wpconfig' ) );
						add_settings_error( 'itsec', esc_attr( 'settings_updated' ), $message, 'error' );
					}
	...
	public function config_metabox_contents() {
		...
		if ( empty( $config ) ) {
			_e( 'There are no rules to write.', 'it-l10n-ithemes-security-pro' );
		} else {
	...
	public function save_rewrites() {
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( 'nginx' === $server ) {
				$retval = array(
					'success' => true,
					'text'    => __( 'You must restart your NGINX server for the settings to take effect', 'it-l10n-ithemes-security-pro' ),
				);
			} else {
				$retval = array(
					'success' => true,
